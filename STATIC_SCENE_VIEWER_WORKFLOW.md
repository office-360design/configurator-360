...
5. The configurator checks the matching URL under `static-site/models/`.
...
6. Run `PREPARE_STATIC_SITE.vbs` or `npm run prepare:static` if other site files changed. The preparation script now preserves `static-site/models/`.
7. Deploy `static-site/` to the existing Netlify site.
...
`ar-upload-config.js` defaults to:
...
- `ar-export.js`: GLB optimization, export, validation, hashing, and download.
- `ar-upload-config.js`: static/API publishing mode and AR limits.
- `index.html`: static publication verification and QR generation.
- `ar-viewer.html`: phone preview and corrected Scene Viewer launch.
- `prepare_static_site.js`: prepares Netlify files without deleting published models.
