...
const FILTER_BY_ABSOLUTE_COORDINATES = false;
const CAD_COORD_LIMIT = 500;
const MAX_LOCAL_GEOMETRY_SIZE = 500;
const PATH_JOIN_TOLERANCE = 0.05;
...
function isPathWithinExportBounds(pathObject, transform) {
    ...
    // Reject only obviously huge individual entities. This protects against
    // layout borders without cutting valid parts according to their placement.
    if ((local.maxX - local.minX) > MAX_LOCAL_GEOMETRY_SIZE ||
        (local.maxY - local.minY) > MAX_LOCAL_GEOMETRY_SIZE) {
        return false;
    }

    // Always validate transformed coordinates, but do not normally clip them
    // to an arbitrary world-space square. A block may legitimately be inserted
    // outside +/-500, or a valid contour may cross that boundary. Filtering one
    // ARC while keeping its neighbouring LINE entities destroys the contour.
    for (const point of pathObject.points) {
        ...
        if (FILTER_BY_ABSOLUTE_COORDINATES &&
            (Math.abs(transformed.x) > CAD_COORD_LIMIT ||
             Math.abs(transformed.y) > CAD_COORD_LIMIT)) {
            return false;
        }
    }
    ...
    function processInsert(ins, parentTransform, parentPath, parentBlockName = null) {
        ...
        for (const entity of block.entities || []) {
            ...
            for (const geometryPath of converted) {
                // Track transformed ranges for diagnosis.
                for (const point of geometryPath.points) {
                    const transformed = currentTransform(point);
                    if (!transformedRangeByType[entity.type]) {
                        transformedRangeByType[entity.type] = {
                            minX: transformed.x,
                            minY: transformed.y,
                            maxX: transformed.x,
                            maxY: transformed.y
                        ...
                        const range = transformedRangeByType[entity.type];
                        range.minX = Math.min(range.minX, transformed.x);
                        range.minY = Math.min(range.minY, transformed.y);
                        range.maxX = Math.max(range.maxX, transformed.x);
                        range.maxY = Math.max(range.maxY, transformed.y);
                    }
                }
        ...
        if (blockName.toLowerCase() === DEBUG_BLOCK_NAME.toLowerCase()) {
            ...
            console.log('Transformed coordinate ranges:', transformedRangeByType);
            console.log('Absolute coordinate clipping enabled:', FILTER_BY_ABSOLUTE_COORDINATES);
