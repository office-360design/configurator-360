...
npx.cmd --yes netlify-cli@latest deploy --prod --dir=dist/site --functions=netlify/functions
```

The included script performs the same deployment:

```text
scripts/windows/deploy_netlify_with_functions.cmd
```
...
- `src/client/ar-upload-config.js` → `maxUsdzBytes`;
...
- the visible action is the `rel="ar"` link on `src/client/ar-viewer.html`.
