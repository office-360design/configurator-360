...
function exportDwgToDxf(dwgFile, dxfFile) {
    ...
    if (fs.existsSync(ACAD_CONSOLE)) {
        ...
        const scrFile = path.join(workspaceDir, 'intermediate', 'temp_export.scr');
...
`;
        ...
        const tempInputDir = path.join(workspaceDir, 'intermediate', 'temp_in');
        const tempOutputDir = path.join(workspaceDir, 'intermediate', 'temp_out');
...
function main() {
    ...
    if (inputExt === '.dwg') {
        ...
        const intermediateDir = path.join(__dirname, 'intermediate');
