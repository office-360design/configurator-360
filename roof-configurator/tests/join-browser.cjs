...
(async () => {
  ...
  const saved = await page.evaluate(async () => {
    const { defaultLayout, splitLayoutInPlace } = await import('../roof-configurator/js/roofLayout.js?v=layout-9');
  ...
  const walls = await page.evaluate(async state => {
    const { buildRoofModel } = await import('../roof-configurator/js/roofFactory.js?v=layout-9');
