...
(async () => {
  ...
  for (const covering of ['generic', 'roca', 'teclado']) {
    const result = await page.evaluate(async covering => {
      const { lShapedLayout, footprintLayout } = await import('../roof-configurator/js/roofLayout.js?v=layout-6');
      const { buildRoofModel } = await import('../roof-configurator/js/roofFactory.js?v=layout-6');
