...
test('mixed colours stay attached to stones across outline and house cuts', () => {
  for (const tile of ['parket', 'tetraNova', 'hbeton'])
    for (const rotation of [0, 90]) {
      ...
      for (const patch of [
        { shape: 'closed4', runA: 6, runB: 4, runC: 4, angleB: 90 },
        { houseEnabled: true, houseX: 1.13, houseZ: 0.79 },
      ]) {
        const parts = layout({ ...state, ...patch });
        let shared = 0;
        for (const p of parts)
          if (original.has(key(p))) {
            assert.deepEqual(stoneAppearance(p, COLORS.noir), original.get(key(p)));
            shared++;
          }
...
test('mixed palette does not repeat every three stones or lock to rows', () => {
  for (const width of [3, 3.1, 3.2, 5.9]) {
    ...
    assert.ok(repeats > 0.2 && repeats < 0.45, `period-three match rate ${repeats}`);
    for (const colour of COLORS.noir) {
      const share = shades.filter((v) => v === colour).length / shades.length;
      assert.ok(share > 0.2 && share < 0.45);
    }
  }
});
test('intentional checker assignment and solid colours are preserved', () => {
  ...
  for (const p of pieces) {
    const palette = p.accent ? COLORS.red : COLORS.grey;
    const appearance = stoneAppearance(p, palette);
    assert.equal(appearance.base, palette);
    assert.ok(appearance.brightness >= 0.96 && appearance.brightness <= 1.04);
  }
});
test('woven mixed finishes retain directional contrast through rotations and cuts', () => {
  for (const pattern of ['herringbone', 'basket'])
    for (const rotation of [0, 90]) {
      const s = normalize({ pattern, rotation, color: 'noir', length: 6, width: 4 });
      const original = new Map(
        layout(s).map((p) => [key(p), stoneAppearance(p, COLORS.noir).base]),
      );
      for (const patch of [
        {},
        { shape: 'closed4', runA: 6, runB: 4, runC: 4, angleB: 90 },
        { shape: 'closed5' },
        { houseEnabled: true, houseShape: 'l', houseX: 0.13, houseZ: 0.27 },
      ]) {
