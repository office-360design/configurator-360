  ...
  for (const pattern of t.patterns)
    for (const rotation of [0, 90])
      test(`${tile} ${pattern} ${rotation}: coverage, clipping and no overlap`, () => {
        ...
        assert.ok(
          e.rows.filter((r) => r.kind === 'tile').reduce((a, r) => a + r.quantity, 0) >= p.length,
        );
      });
test('BOM reference: 2m square paving, no waste, no curb', () => {
  const s = {
    ...DEFAULTS,
    tile: 'square',
    pattern: 'stack',
  ...
  const e = estimate(s);
  assert.equal(e.installedPieces, 100);
  assert.equal(e.cutPieces, 0);
  assert.equal(e.rows[0].quantity, 100);
  assert.ok(Math.abs(e.total - 400) < 1e-8);
});
  ...
  for (const curb of Object.keys(CURBS))
    for (let mask = 0; mask < 16; mask++) {
      const s = normalize({
          ...
          length: 1.1,
          width: 1.3,
          edges: [0, 1, 2, 3].map((i) => Boolean(mask & (1 << i))),
        }),
        parts = curbLayout(s),
        c = CURBS[curb];
      const expected = [0, 1, 2, 3].reduce(
        (n, side) =>
          n +
          (s.edges[side]
            ? Math.ceil(
                ((side % 2
                  ? s.width
                  : s.length + c.width * (Number(s.edges[1]) + Number(s.edges[3]))) -
                  1e-9) /
                  c.length,
              )
            : 0),
...
test('spares, rate changes and checker colour split', () => {
  const s = {
    ...DEFAULTS,
    tile: 'square',
    pattern: 'checker',
  ...
  const e = estimate(s);
  assert.deepEqual(
    e.rows.map((r) => r.quantity),
    [55, 55],
  );
  assert.ok(Math.abs(e.total - 440) < 1e-8);
  assert.equal(estimate({ ...s, tileRate: 0 }).total, 0);
});
test('untrusted saved-state limits and compatible patterns', () => {
  const s = normalize({
    length: Infinity,
    width: -5,
    tile: '__proto__',
  ...
  assert.equal(s.tile, 'parket');
  assert.equal(s.waste, 30);
  assert.deepEqual(s.edges, [false, true, true, true]);
  assert.equal(normalize({ tile: 'square', pattern: 'herringbone' }).pattern, 'stack');
});
