...
export function houseGeometry(s){
  ...
  const outline=s.houseShape==='l'?[{x:0,z:0},{x:l,z:0},{x:l,z:b},{x:a,z:b},{x:a,z:w},{x:0,z:w}]:rectanglePoints(raw[0]);
  ...
  return {rectangles,outline:outline.map(transform)};
}
...
export function excludeHouse(pieces,s,{curbWidth}={}){
  const rectangles=houseGeometry(s).rectangles;if(!rectangles.length)return pieces;
  return pieces.flatMap(p=>{
    const polygons=p.fragments||[p.polygon||rectanglePoints({x:p.x-p.l/2,z:p.z-p.w/2,l:p.l,w:p.w})];
    const original=polygons.reduce((a,poly)=>a+polygonArea(poly),0),fragments=subtractHouse(polygons,rectangles),area=fragments.reduce((a,poly)=>a+polygonArea(poly),0);
    if(area<1e-8)return [];
