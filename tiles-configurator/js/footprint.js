...
export function normalizeFootprint(value){
  ...
  for(let i=0;i<p.length;i++){
    const a=p[i],b=p[(i+1)%p.length];if(Math.hypot(a.x-b.x,a.z-b.z)<1e-6)return null;
    for(let j=i+2;j<p.length;j++){
      if((j+1)%p.length===i)continue;
      const c=p[j],d=p[(j+1)%p.length];
      if(cross(a,b,c)*cross(a,b,d)<=0&&cross(c,d,a)*cross(c,d,b)<=0&&Math.max(a.x,b.x)>=Math.min(c.x,d.x)&&Math.max(c.x,d.x)>=Math.min(a.x,b.x)&&Math.max(a.z,b.z)>=Math.min(c.z,d.z)&&Math.max(c.z,d.z)>=Math.min(a.z,b.z))return null;
    }
  }
  const minX=Math.min(...p.map(v=>v.x)),minZ=Math.min(...p.map(v=>v.z));p=p.map(v=>({x:v.x-minX,z:v.z-minZ}));
  const width=Math.max(...p.map(v=>v.x)),depth=Math.max(...p.map(v=>v.z));
  if(width<1||depth<1||width>20||depth>20||polygonArea(p)<1)return null;
  if(p.reduce((s,v,i)=>s+v.x*p[(i+1)%p.length].z-v.z*p[(i+1)%p.length].x,0)<0)p.reverse();
  try{triangulate(p);}catch{return null;}return p;
}
export function mappedFootprint(geometry){
  ...
  return normalizeFootprint(geometry.map(v=>({x:(v.lon-first.lon)*111320*Math.cos(first.lat*Math.PI/180),z:(first.lat-v.lat)*111320})));
}
function halfPlane(poly,a,b,inside){
  const out=[],sign=inside?1:-1;
  for(let i=0;i<poly.length;i++){
    const p=poly[i],q=poly[(i+1)%poly.length],dp=cross(a,b,p)*sign,dq=cross(a,b,q)*sign;
    if(dp>=0)out.push(p);
    if((dp>=0)!==(dq>=0)){const t=dp/(dp-dq);out.push({x:p.x+t*(q.x-p.x),z:p.z+t*(q.z-p.z)});}
  }
  return out;
}
...
export function subtractFootprint(polygons,triangles){
  let result=polygons;
  for(const tri of triangles)result=result.flatMap(poly=>{
    let inside=poly;const outside=[];
    for(let i=0;i<3&&inside.length;i++){
      const a=tri[i],b=tri[(i+1)%3],part=halfPlane(inside,a,b,false);
