...
export function createViewer(host){
  ...
  function fit(){if(!state)return;const size=Math.max(bounds.width,bounds.depth),distance=size*1.55/Math.min(1,camera.aspect);controls.target.set(0,0,0);camera.position.set(top?0:distance*.8,top?distance*1.5:distance,top?.001:distance);controls.update();}
  ...
  function rebuild(s,parts){
    ...
    drawPieces(curbs.filter(p=>!p.polygon),curb.height,()=>s.curbColor,t.thickness+.035-curb.height);
    drawPolygons(curbs.filter(p=>p.polygon).map((p,index)=>{
