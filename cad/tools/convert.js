...
const SOURCE_DIR = path.join(WORKSPACE_DIR, 'source');
const INTERMEDIATE_DIR = path.join(WORKSPACE_DIR, 'intermediate');
const SVG_DIR = path.join(PROJECT_ROOT, 'src', 'client', 'svg');
...
function run() {
    ...
    function tryExport(name) {
        ...
        const DWG_FILE = isDxf ? null : path.resolve(SOURCE_DIR, dwgBase + '.dwg');
        const DXF_FILE = isDxf ? path.resolve(SOURCE_DIR, name) : path.resolve(INTERMEDIATE_DIR, dwgBase + '.dxf');
        ...
        if (fs.existsSync(ACAD_CONSOLE)) {
            ...
            const SCR_FILE = path.join(INTERMEDIATE_DIR, 'temp_export.scr');
...
`;
    ...
    let exportResult;
    try {
        const origDwgFile = path.resolve(SOURCE_DIR, dwgName);
        if (!fs.existsSync(origDwgFile)) {
            const sanitized = sanitizeFilename(dwgName);
            const checkPath = path.resolve(SOURCE_DIR, sanitized);
        ...
        if (sanitized !== dwgName) {
            const origPath = path.join(SOURCE_DIR, dwgName);
            const sanitizedPath = path.join(SOURCE_DIR, sanitized);
