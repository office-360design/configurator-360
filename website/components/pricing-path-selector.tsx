...
export function PricingPathSelector({ locale }: { locale: Locale }) {
  ...
  return <div className="deployment-selector">
    ...
    <article className="deployment-panel" role="tabpanel" id={`deployment-panel-${path.id}`} aria-labelledby={`deployment-tab-${path.id}`}>
      ...
      <div className="deployment-spec"><div className="deployment-fit"><p>{path.fit}</p>{active === 0 && <PricingSolid shape="d10" items={path.items} description={path.summary} label={`${path.name} implementation features`}/>}</div><ol>{path.items.map((item, index) => <li key={item}><span>{String(index + 1).padStart(2, "0")}</span>{item}</li>)}</ol></div>
