...
export function ContactForm({ locale }: { locale: Locale }) {
  ...
  const copy = locale === "ro" ? {
    name: "Nume", workEmail: "Email profesional", company: "Companie", product: "Ce produs doriți să configurați?", message: "Descrieți produsul, regulile și rezultatul dorit", submit: "Pregătește mesajul", note: "La trimitere se deschide aplicația ta de email, cu mesajul completat și destinatarul corect.", ready: "Mesajul este pregătit în aplicația de email.", subject: "Proiect nou 360Configurator", path: "Traseu comercial selectat",
  } : locale === "de" ? {
    name: "Name", workEmail: "Geschäftliche E-Mail", company: "Unternehmen", product: "Welches Produkt möchten Sie konfigurieren?", message: "Beschreiben Sie Produkt, Regeln und gewünschtes Ergebnis", submit: "Nachricht vorbereiten", note: "Beim Absenden öffnet sich Ihre E-Mail-App mit ausgefüllter Nachricht und korrektem Empfänger.", ready: "Die Nachricht wurde in Ihrer E-Mail-App vorbereitet.", subject: "Neues 360Configurator-Projekt", path: "Gewählter Implementierungsweg",
  } : {
    name: "Name", workEmail: "Work email", company: "Company", product: "What product do you want to configure?", message: "Describe the product, its rules and the outcome you need", submit: "Prepare message", note: "Submitting opens your email app with the message completed and addressed correctly.", ready: "Your message is ready in your email app.", subject: "New 360Configurator project", path: "Selected commercial path",
  };
  useEffect(() => {
    const timer = window.setTimeout(() => setProjectPath((new URLSearchParams(window.location.search).get("project") || "").replaceAll("-", " ")), 0);
  ...
  return <form className="contact-form" onSubmit={(event) => {
    ...
    const body = [
      ...
      ...(projectPath ? ["", `${copy.path}: ${projectPath}`] : []), "", `${copy.product}:`, String(data.get("product") || ""), "", `${copy.message}:`, String(data.get("message") || ""),
    ].join("\n");
    setSent(true);
