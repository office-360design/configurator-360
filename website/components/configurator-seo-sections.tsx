...
// Keep the large localized SEO section behind a Client Component boundary.
...
export function ConfiguratorSeoSections({
  locale,
  slug,
  launchUrl,
}: {
  ...
  const copy = configuratorSeoContent[slug][locale];
  const workflowId = `${slug}-how-it-works`;

  return <>
    <section className="pergola-search-intro solid-section" id={`${slug}-configurator-overview`}>
      <div className="page-frame pergola-search-intro-grid" data-reveal>
        ...
        <div>
          ...
          <div className="pergola-search-actions">
            ...
            <a className="text-link" href={`#${workflowId}`}>{copy.intro.learn} <span>↓</span></a>
          </div>
        </div>
      </div>
    </section>

    <section className="pergola-workflow solid-section" id={workflowId}>
