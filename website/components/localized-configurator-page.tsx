...
import { PergolaSeoSections } from "./pergola-seo-sections";
...
export function LocalizedConfiguratorPage({ locale, slug }: { locale: Locale; slug: string }) {
  ...
  const structuredData = graphSchema([
    ...
    },
    {
      ...
      name: item.title,
      url: item.slug === "pergola" ? item.launchUrl : absoluteUrl(locale, `/configurators/${item.slug}`),
  ...
  return <main className={`site-shell detail-page detail-${item.slug}`}>
    ...
    {item.slug === "pergola" && <PergolaSeoSections locale={locale} launchUrl={item.launchUrl} />}
