...
import { NewProductSeo } from "./new-product-seo";
...
export function LocalizedConfiguratorPage({ locale, slug }: { locale: Locale; slug: string }) {
  ...
  const structuredData = graphSchema([
    ...(isLiteSlug(item.slug) ? [{
      "@type": "FAQPage",
      "@id": `${absoluteUrl(locale, `/configurators/${item.slug}`)}#faq`,
      inLanguage: locale,
      mainEntity: newProductCopy[locale][item.slug].questions.map(([question, answer]) => ({
    ...
    websiteSchema(locale, item.seoDescription),
    {
      ...
      mainEntity: { "@id": `${item.launchUrl}#application` },
    },
    {
      "@type": "WebApplication",
      "@id": `${item.launchUrl}#application`,
      name: item.title,
      url: item.launchUrl,
  ...
  return <main className={`site-shell detail-page detail-${item.slug}${isLiteSlug(item.slug) ? " detail-lite" : ""}`}>
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} />
    <SmoothScroll />{!isLiteSlug(item.slug) && <DeferredWebGLStage />}<SiteHeader locale={locale} currentPath={path} />
    <section className="detail-hero" data-scene={isLiteSlug(item.slug) ? undefined : item.slug}><div className="detail-hero-inner page-frame">
      ...
      <div className="detail-viewport">
        {isLiteSlug(item.slug) ? <LitePreview slug={item.slug} locale={locale} /> : item.slug === "window" ? <WindowConfiguratorPreview locale={locale} placement="detail" /> : <SceneInteractor scene={item.slug} locale={locale} />}
    ...
    {isLiteSlug(item.slug) ? <NewProductSeo locale={locale} slug={item.slug} /> : <ConfiguratorSeoSections locale={locale} slug={item.slug} launchUrl={item.launchUrl} />}
